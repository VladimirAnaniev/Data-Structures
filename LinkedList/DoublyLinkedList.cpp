...
bool isPalindrome(DoubleLinkedList<T>& l) {
    ...
    return !nit || *nit == *pit || *(nit.prev()) == *pit;
}

template <typename T>
bool isNestedPalindrome(DoubleLinkedList<DoubleLinkedList<T>>& ll) {
    DoubleLinkedListIterator<DoubleLinkedList<int>> iter = ll.begin();
    DoubleLinkedList<int> flattened;

    while(iter) {
        flattened.append(iter.get());
        iter++;
    }

    return isPalindrome(flattened);
}
